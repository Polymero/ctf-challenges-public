...
class NTTDomain:
    ...
    def INTT(self, pts: list) -> list:
        """ Inverse Number Theoretic Transform (INTT) transformation. """
        n = len(pts)
        nInv = inverse(n, self.q)
        poly = self.NTT(pts, self.wInv)
        for i in range(n):
            poly[i] = (poly[i] * nInv) % self.q
        return poly
...
class NTTPoints:
    ...
    def toPoly(self) -> list:
        return [(j * pow(self.domain.pInv, i, self.domain.q)) % self.domain.q for i,j in enumerate(self.domain.INTT(self.pts))]
